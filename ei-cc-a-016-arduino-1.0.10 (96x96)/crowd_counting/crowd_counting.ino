...
/* Includes ---------------------------------------------------------------- */
#include <Crowd_Counting_Portenta_A-016_inferencing.h>
 #include "camera.h"
#include "himax.h"
...
*/
int image_count = 0;
constexpr int images_to_write = 500;
bool sd_card_initialized = false;

char filename[255];

// Resultado de Crowd Counting
int crowd_count_act = 0;
int crowd_count_avg = 0;
...
void setup()
{
    ...
    Serial.begin(115200);
    if (DEBUG){
      // esperar que monitor de serial este disponible
      while (!Serial);
...
void loop()
{
    ...
    //if (DEBUG){
        Serial.print("\n* * * Crowd count actual: ");
        Serial.print(crowd_count_act);
        Serial.println("* * * * * * * * * * *\n");
    //}

    crowd_count_avg += crowd_count_act;
    
    // Crowd count promedio
    // y envio por LoRa
    if ((image_count + 1) % 3 == 0){

      // promedio de ultimas 3 detecciones
      crowd_count_avg /= 3;
      
      sprintf(crowd_count_str, "%03d", crowd_count_avg);
      //if (DEBUG){
        Serial.print("\n* * * Crowd count promedio: ");
        Serial.print(crowd_count_avg);
        Serial.println("* * * * * * * * * * *\n");
      ...
      // envio por LoRa mediante modulo Murata
...
bool ei_camera_init(void) {
    ...
    #ifdef EI_CAMERA_FRAME_BUFFER_SDRAM
        // en este caso esa memoria no se esta liberando
...
bool ei_camera_capture(uint32_t img_width, uint32_t img_height, uint8_t *out_buf) {
    ...
    if(sd_card_initialized && image_count < images_to_write) {
      ...
      saveImage(ei_camera_capture_out, filename, image_count);
    }
